...
const Checkout = ({ subTotal }) => {
  const { loading, error, success } = useSelector((state) => state.placeOrderReducer);
        ...
        currentUser ? (
          ...
          >
            <button className="btn btn-danger">
              {loading ? "Order Processing" : "Pay Now"}
            </button>
          </StripeCheckout>
        ) : success ? (
          <Link to="/"> 
          ...
          <Link to="/cart/confirm">
            <button className="btn btn-danger">To pay Login First</button>
          </Link>
        )
      }
    </div>
